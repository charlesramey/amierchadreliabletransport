...
class SenderAPI:
    ...
    def handshake(self, send_socket):

        handshakeSocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        rcvr = threading.Thread(target=self.unrelReceiver, args=(handshakeSocket, '127.0.0.1', 6005))
        rcvr.start()
